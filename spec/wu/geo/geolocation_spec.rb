...
describe Wu::Geo::Geolocation do
  ...
  context Wu::Geo::Geolocation do
    ...
    it "returns a map tile size given a zoom level" do
      Wu::Geo::Geolocation.map_tile_size(3).should == 8
    end
    ...
    it "returns tile x-y pair and a zoom level given a quadkey" do
      Wu::Geo::Geolocation.quadkey_to_tile_xy_zl(aus_quadkey[0..2] ).should == [1, 3, 3]
      Wu::Geo::Geolocation.quadkey_to_tile_xy_zl(aus_quadkey[0..7] ).should == [aus_tile_x_8.floor,  aus_tile_y_8.floor,  8]
      Wu::Geo::Geolocation.quadkey_to_tile_xy_zl(aus_quadkey[0..10]).should == [aus_tile_x_11.floor, aus_tile_y_11.floor, 11]
    end

    it "allows '' to be a quadkey (whole map)" do
      Wu::Geo::Geolocation.quadkey_to_tile_xy_zl("").should == [0, 0, 0]
    end
    ...
    it "returns a bounding box given a point and radius" do
      left, btm, right, top = Wu::Geo::Geolocation.lng_lat_rad_to_bbox(aus_lng, aus_lat, radius)
    ...
    it "returns a centroid given a bounding box" do
      mid_lng, mid_lat = Wu::Geo::Geolocation.bbox_centroid([aus_lng, sat_lat], [sat_lng, aus_lat])
    ...
    it "returns a pixel resolution given a latitude and zoom level" do
      Wu::Geo::Geolocation.pixel_resolution(aus_lat, 3).should be_within(0.0001).of(16880.4081)
    end

    it "returns a map scale given a latitude, zoom level and dpi" do
      Wu::Geo::Geolocation.map_scale_for_dpi(aus_lat, 3, dpi).should be_within(0.0001).of(47849975.8302)
    end

    it "calculates the haversine distance between two points" do
      Wu::Geo::Geolocation.haversine_distance(aus_lng, aus_lat, sat_lng, sat_lat).should be_within(0.0001).of(117522.1219)
    end

    it "calculates the haversine midpoint between two points" do
      lng, lat = Wu::Geo::Geolocation.haversine_midpoint(aus_lng, sat_lat, sat_lng, aus_lat)
    ...
    it "calculates the point a given distance directly north from a lat/lng" do
      lng, lat = Wu::Geo::Geolocation.point_north(aus_lng, aus_lat, 1000000)
    ...
    it "calculates the point a given distance directly east from a lat/lng" do
      lng, lat = Wu::Geo::Geolocation.point_east(aus_lng, aus_lat, 1000000)
  ...
  context Wu::Geo::Geolocated::ByCoordinates do
    let(:point_klass) do
      module Wukong
        class TestPoint
          include Gorillib::Model
          include Wu::Geo::Geolocated::ByCoordinates
