...
describe "Reducers" do
  describe :group do
    ...
    let(:grouped_nums)    { [[nil, 2], [1, 1], [5, 1], [10, 1], [100, 1]] }
    ...
    it "can group from within a JSON hash" do
      proc = processor(:group, by: 'data.n').given(*json_sorted_n).should emit(*grouped_nums)
    end
    it "can group from within a TSV row" do
      proc = processor(:group, by: '3').given(*tsv_sorted).should emit(*grouped_nums)
    end
  end
end
