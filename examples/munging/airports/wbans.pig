// Outputs a list of WBAN IDs assigned to airports
...
mshr_grouped = GROUP mshr BY (icao_id, wban_id, faa_id);
mshr_flattened = FOREACH mshr_grouped GENERATE FLATTEN(group) AS (wban_id, icao_id, faa_id);
mshr_filtered = FILTER mshr_flattened BY (icao_id is not null and wban_id is not null);

mshr_final = FOREACH mshr_filtered GENERATE wban_id;
STORE mshr_final INTO '/Users/dlaw/Desktop/stations/wbans';
